...
public class RallyToJiraSpecial {
	...
	private JsonElement createTasks(JsonObject workspace, JsonObject project, JsonObject userStory) throws IOException, Exception {
		int a = 1;
		JsonElement tasksObj = userStory.get("Tasks");
		if (!tasksObj.isJsonNull()) {
			JsonArray tasks = tasksObj.getAsJsonArray();
			for (JsonElement jeTask : tasks) {
				System.out.println("**TASK " + a++ + " of " + tasks.size() + " *************************************");
				JsonObject task = rally.findRallyObjectByObjectID(RallyObject.TASK, jeTask.getAsJsonObject().get("ObjectID").getAsString());
